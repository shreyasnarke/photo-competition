...
<%
    ...
    String folderName = "admin-theme-photo";
    try{
        ...
        changeFileName(fileName,  uploadFolderPath + folderName, out);
...
<%!
    public static void changeFileName(String fileName, String path, JspWriter out) throws Exception{
        try{
            ...
            storePhotoDetails(path + File.separator + newFileName, out);
        } catch(Exception e){
            out.println(" isDirectoryExists: " +e);
        }
    }
...
<%!
    public static void storePhotoDetails(String path, JspWriter out) throws Exception{
        try{
            String themeId = generatePrimaryKey("T", "theme", "theme_id", out);
            con = fetchConnection();
            stmt = con.createStatement();
            String query = "insert into theme(theme_id, theme_photo) values('"+themeId+"', '"+path+"')";
...
<%!
	public static String generateNewFileName(){
		...
		return "THEME-" + currentDate.getYear() + "" + currentDate.getMonthValue() + "" + currentDate.getDayOfMonth() + "" +
